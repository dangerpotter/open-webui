		...
		<div class="w-full sm:max-w-md px-10 min-h-screen flex flex-col text-center">
			{#if ($config?.features.auth_trusted_header ?? false) || $config?.features.auth === false}
				<div class=" my-auto pb-10 w-full">
					...
					>
						<div>
							{$i18n.t('Signing in to {{Falcor_NAME}}', { Falcor_NAME: $Falcor_NAME })}
						</div>
					...
					>
						<div class="mb-1">
							<div class=" text-2xl font-medium">
								{#if mode === 'signin'}
									{$i18n.t(`Sign in to {{Falcor_NAME}}`, { Falcor_NAME: $Falcor_NAME })}
								{:else}
									{$i18n.t(`Sign up to {{Falcor_NAME}}`, { Falcor_NAME: $Falcor_NAME })}
								{/if}
							</div>
