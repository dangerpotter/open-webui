...
usage() {
    ...
    echo "  --Falcor[port=PORT]         Set the port for the web user interface."
    ...
    echo "  $0 --enable-gpu[count=1] --enable-api[port=12345] --Falcor[port=3000]"
    echo "  $0 --enable-gpu[count=1] --enable-api[port=12345] --Falcor[port=3000] --data[folder=./ollama-data]"
    echo "  $0 --enable-gpu[count=1] --enable-api[port=12345] --Falcor[port=3000] --data[folder=./ollama-data] --build"
    echo ""
...
if [[ $kill_compose == true ]]; then
    ...
    if [[ -n $Falcor_port ]]; then
        export Falcor_PORT=$Falcor_port # Set Falcor_PORT environment variable
    fi
...
echo -e "   ${GREEN}${BOLD}Falcor Port:${NC} $Falcor_port"
echo
