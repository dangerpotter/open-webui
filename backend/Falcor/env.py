...
Falcor_DIR = Path(__file__).parent  # the path containing this file
print(Falcor_DIR)

BACKEND_DIR = Falcor_DIR.parent  # the path containing this file
...
try:
    ...
    changelog_content = (pkgutil.get_data("Falcor", "CHANGELOG.md") or b"").decode()
...
if FROM_INIT_PY:
    NEW_DATA_DIR = Path(os.getenv("DATA_DIR", Falcor_DIR / "data")).resolve()
    ...
    if DATA_DIR.exists() and DATA_DIR != NEW_DATA_DIR:
        ...
        shutil.make_archive(DATA_DIR.parent / "Falcor_data", "zip", DATA_DIR)

        # Remove the old data directory
        shutil.rmtree(DATA_DIR)

    DATA_DIR = Path(os.getenv("DATA_DIR", Falcor_DIR / "data"))


FONTS_DIR = Path(os.getenv("FONTS_DIR", Falcor_DIR / "static" / "fonts"))

FRONTEND_BUILD_DIR = Path(os.getenv("FRONTEND_BUILD_DIR", BASE_DIR / "build")).resolve()

if FROM_INIT_PY:
    FRONTEND_BUILD_DIR = Path(
        os.getenv("FRONTEND_BUILD_DIR", Falcor_DIR / "frontend")
    ).resolve()
