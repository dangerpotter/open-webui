...
if [ -n "$SPACE_ID" ]; then
  ...
  if [ -n "$ADMIN_USER_EMAIL" ] && [ -n "$ADMIN_USER_PASSWORD" ]; then
    ...
    Falcor_SECRET_KEY="$Falcor_SECRET_KEY" uvicorn Falcor.main:app --host "$HOST" --port "$PORT" --forwarded-allow-ips '*' &
    Falcor_pid=$!
...
Falcor_SECRET_KEY="$Falcor_SECRET_KEY" exec uvicorn Falcor.main:app --host "$HOST" --port "$PORT" --forwarded-allow-ips '*'
